...
export default function App() {
  ...
  // Estados de datos con valores iniciales seguros para evitar "undefined"
  const [metaData, setMetaData] = useState({
    metrics: { leads: 0, metaLeads: 1, budget: 0, spent: 0 },
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (snap) => {
      if (snap.exists()) {
        ...
        // Validaciones de seguridad al cargar para evitar errores de propiedades inexistentes
        if (data.metaData) {
            setMetaData({
                metrics: data.metaData.metrics || { leads: 0, metaLeads: 1, budget: 0, spent: 0 },
                checklists: data.metaData.checklists || { awareness: [], prospeccion: [], retargeting: [] },
                drive: data.metaData.drive || { awareness: '', prospeccion: '', retargeting: '' }
            });
        }
  ...
  if (!user || loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
      ...
      <p className="text-slate-400 font-medium animate-pulse">Sincronizando con Matrix Hub...</p>
    </div>
  );
            ...
            <div className="space-y-10 animate-in fade-in duration-500">
              <header className="flex justify-between items-end">
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-4 py-2 border rounded-lg text-sm font-semibold shadow-sm transition-all ${isEditingMetrics ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50'}`}
                >
                  {isEditingMetrics ? <><Save size={16}/> Guardar</> : <><Edit2 size={16}/> Editar Métricas</>}
                </button>
              </header>

              {/* Métricas Principales - Usando Optional Chaining ?. para seguridad */}
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <MetricCard 
                  label="Leads Generados" 
                  value={metaData?.metrics?.leads ?? 0} 
                ...
                <MetricCard 
                  label="Meta de Leads" 
                  value={metaData?.metrics?.metaLeads ?? 1} 
                  subtext={`${Math.round(((metaData?.metrics?.leads || 0) / (metaData?.metrics?.metaLeads || 1)) * 100)}% alcanzado`}
                ...
                <MetricCard 
                  ...
                  subtext={`${Math.round(((metaData?.metrics?.spent || 0) / (metaData?.metrics?.budget || 1)) * 100)}% usado`}
          ...
          {activeTab === 'eventos' && (
            <div className="space-y-8 animate-in slide-in-from-right duration-500">
              ...
              <div className="grid grid-cols-1 gap-8">
                {eventos.length === 0 ? (
                  <div className="py-20 text-center text-slate-400 font-medium bg-white rounded-3xl border-2 border-dashed border-slate-100">No hay eventos creados. Selecciona o crea uno para comenzar.</div>
                ) : (
                  eventos.map(ev => (
...
function MobileIcon({ active, icon, onClick }) {
  return (
    <button onClick={onClick} className={`p-2 rounded-lg transition-colors ${active ? 'text-blue-600 bg-blue-50' : 'text-slate-400 hover:text-slate-600'}`}>
      ...
      <div className="space-y-2 flex-1">
        ...
        {editing ? (
          <div className="relative">
            {isCurrency && <span className="absolute left-0 top-1/2 -translate-y-1/2 text-slate-400">$</span>}
            <input 
              type="number" 
              className={`w-full text-xl font-black text-slate-800 border-b-2 border-blue-500 outline-none ${isCurrency ? 'pl-4' : ''}`} 
      ...
      <div className="space-y-3 min-h-[100px]">
        ...
        <div className="flex gap-2 mt-4">
          <input 
            ...
            className="flex-1 text-xs p-2 bg-slate-50 border border-slate-100 rounded-lg outline-none focus:ring-1 focus:ring-blue-500"
            onKeyDown={e => {
          ...
          }} className="text-slate-400 hover:text-blue-500 transition-colors"><Plus size={16}/></button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <button 
          ...
          className="bg-blue-600 text-white px-6 py-2 rounded-xl text-xs font-bold shadow-md shadow-blue-50 hover:bg-blue-700 transition-all"
        >
          Conectar
        </button>
      </div>
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          {files.map(f => (
            <div key={f.id} className="min-w-[120px] aspect-square bg-slate-50 rounded-xl overflow-hidden relative group">
              ...
              <a href={f.webContentLink} target="_blank" rel="noopener noreferrer" className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
...
function EventCard({ ev, onUpdate, onDelete }) {
  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8 animate-in zoom-in-95 duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100 space-y-6">
          <div className="flex items-center justify-between">
            <h5 className="font-bold flex items-center gap-2 text-slate-700"><CheckCircle2 size={18} className="text-blue-500"/> Tareas</h5>
            <span className="bg-white px-3 py-1 rounded-full text-[10px] font-black text-slate-400 border border-slate-100">
              {ev?.tareas?.filter(t => t.done).length || 0}/{ev?.tareas?.length || 0}
            </span>
          </div>
          <div className="space-y-3">
              {ev?.tareas?.map((t, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  ...
                  <button onClick={() => onUpdate({...ev, tareas: ev.tareas.filter((_, i) => i !== idx)})} className="text-slate-200 hover:text-rose-400 opacity-0 group-hover:opacity-100 transition-all"><X size={14}/></button>
                </div>
              ))}
              <div className="flex gap-2 pt-2">
                 <input className="flex-1 bg-white p-3 rounded-xl border border-slate-200 text-sm font-medium outline-none focus:ring-1 focus:ring-blue-500" placeholder="Nueva tarea..." value={task} onChange={e => setTask(e.target.value)} onKeyDown={e => {
                   if(e.key === 'Enter' && task) {
                     onUpdate({...ev, tareas: [...(ev.tareas || []), { text: task, done: false }]});
                     setTask('');
                   }
                 }}/>
                 <button onClick={() => { if(task) { onUpdate({...ev, tareas: [...(ev.tareas || []), { text: task, done: false }]}); setTask(''); } }} className="bg-white p-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors"><Plus size={18}/></button>
              </div>
          </div>
        </div>
        ...
        <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100 space-y-6">
          <div className="flex items-center justify-between">
            <h5 className="font-bold flex items-center gap-2 text-slate-700"><DollarSign size={18} className="text-amber-500"/> Gastos / Actividades</h5>
            <span className="bg-white px-3 py-1 rounded-full text-[10px] font-black text-slate-400 border border-slate-100">
              ${(ev?.gastos?.reduce((a, b) => a + (parseFloat(b.costo) || 0), 0) || 0).toLocaleString()}
            </span>
          </div>
          <div className="space-y-3">
              {ev?.gastos?.map((g, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl shadow-sm group">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-black text-slate-800">${(g.costo || 0).toLocaleString()}</span>
                    <button onClick={() => onUpdate({...ev, gastos: ev.gastos.filter((_, i) => i !== idx)})} className="text-slate-200 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"><X size={14}/></button>
                  </div>
                </div>
              ))}
              <div className="flex gap-2 pt-2">
                 <input className="flex-1 bg-white p-3 rounded-xl border border-slate-200 text-sm font-medium outline-none focus:ring-1 focus:ring-blue-500" placeholder="Actividad..." value={act} onChange={e => setAct(e.target.value)}/>
                 <input className="w-24 bg-white p-3 rounded-xl border border-slate-200 text-sm font-medium outline-none focus:ring-1 focus:ring-blue-500" placeholder="$" type="number" value={price} onChange={e => setPrice(e.target.value)}/>
                 <button onClick={() => {
                   if(act && price) {
                     onUpdate({...ev, gastos: [...(ev.gastos || []), { text: act, costo: parseFloat(price) }]});
                     setAct(''); setPrice('');
                   }
                 }} className="bg-white p-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors"><Plus size={18}/></button>
              </div>
          </div>
        </div>
      </div>
...
function InsumoCard({ item }) {
  // Cálculo de días restantes (simulado para este ejemplo)
  const daysLeft = 15; 
    ...
    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 text-[11px] font-bold text-slate-400 uppercase tracking-widest">
          <MapPin size={12}/> {item.sucursal || 'Sucursal Centro'}
        </div>
        <div className="flex items-center gap-2 text-[11px] font-bold text-slate-400 uppercase tracking-widest">
          <Calendar size={12}/> Próx. Renovación: 19 abr
        </div>
      </div>
      ...
      <div className="mt-6 flex justify-between items-center">
        ...
        <span className="text-[10px] font-bold text-slate-400">Cada {item.dias} días</span>
      </div>
    </div>
  );
}
...
function ExpressTask({ task, onToggle, onDelete }) {
  return (
    <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all flex items-center justify-between group animate-in slide-in-from-left duration-300">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="flex gap-4 mt-1">
            <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${task.priority === 'Alta' ? 'bg-rose-50 text-rose-500' : 'bg-slate-50 text-slate-400'}`}>{task.priority || 'Media'}</span>
      ...
      <button 
        onClick={() => { if(confirm('¿Eliminar tarea?')) onDelete(); }}
        className="opacity-0 group-hover:opacity-100 text-slate-200 hover:text-rose-500 transition-all p-2"
      >
          <Trash2 size={18} />
      </button>
    </div>
...
// Icons con manejo de colores consistente
...
function ChartIcon({color}) { return <div className={`p-2 rounded-xl ${color}`}><Zap size={20}/></div> }
function MapPinIcon({color}) { return <div className={`p-2 rounded-xl ${color}`}><MapPin size={20}/></div> }
