...
export default function App() {
  ...
  useEffect(() => {
    return onAuthStateChanged(auth, (u) => {
      setUser(u);
      if (u) {
        // Simulación de roles por email o anonimato
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Intento anónimo si falla el login tradicional para demo
  ...
  const currentMeta = metaBranches[sucursalMeta];
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <MetricCard 
                  ...
                  onChange={(v) => {
                    ...
                    updated[sucursalMeta].metrics.leads = v;
                ...
                <MetricCard 
                  ...
                  onChange={(v) => {
                    const updated = { ...metaBranches };
                    updated[sucursalMeta].metrics.metaLeads = v;
                    setMetaBranches(updated);
                  }}
                />
                <MetricCard 
                  ...
                  onChange={(v) => {
                    const updated = { ...metaBranches };
                    updated[sucursalMeta].metrics.budget = v;
                    setMetaBranches(updated);
                  }}
                />
                <MetricCard 
                  ...
                  onChange={(v) => {
                    const updated = { ...metaBranches };
                    updated[sucursalMeta].metrics.spent = v;
                    setMetaBranches(updated);
                  }}
                />
              </div>
...
const TargetIcon = ({ color }) => <div className={`p-2 rounded-xl ${color}`}><Zap size={20}/></div>;
